...
const TokenVerifier = ({
  ...
  const {token, resendData, isValidToken, displayResendFields, isValidUser} =
    tokenData;
  const {email, password} = resendData;
  const {fromScreen} = params;
  const tokenKey =
    fromScreen === 'user' ? 'x-auth-token' : 'x-auth-admin-token';
  ...
  const onLinkClick = () =>
    setTokenData({...tokenData, displayResendFields: !displayResendFields});
  ...
  const onSubmitForm = async (e) => {
    ...
    try {
      ...
      setTimeout(
        () => navigate(fromScreen === 'user' ? '/login' : '/adminLogin'),
        12000
      );
    } catch (error) {
  ...
  return isAuthenticated && !activateAdminNavLinks ? ( //TODO componentDidMount of Admin is stored in redux-state and can be used here for navigation
    ...
    <Fragment>
      ...
      <p className="lead">
        <i className="fas fa-paper-plane"></i> Check for Token in your Inbox of
        E-Mail you provided
      </p>
      {showTimer && <Timer />}
      <form className="form" onSubmit={(e) => onSubmitForm(e)}>
        <div className="form-group">
          ...
          />
          {!isValidToken && (
            <small className="form-danger">Invalid token.</small>
          )}
        </div>
        <button
          type="submit"
      ...
      {displayResendFields && (
        <Fragment>
          ...
          <br />
          <p className="lead">
            <i className="fas fa-envelope-open-text"></i> Enter registered
            E-mail to get token again
          </p>
          <form className="form" onSubmit={(e) => onSubmitResendForm(e)}>
            ...
            <div className="form-group">
              ...
              />
              {!isValidUser && (
                <small className="form-danger">Invalid credentials.</small>
              )}
            </div>
            <button
              type="submit"
