...
const ShowProfile = ({profile, getCurrentProfile, setAlert}) => {
  ...
  const {isAboutEditEnabled, fieldCurrAddress, isEditEnabled, isValidMobNumb, isValidAltCont, isModalVisible, modalJSX, payload} = componentState;
  ...
  const editInfo = () => {
    setComponentState({...componentState, isEditEnabled: !isEditEnabled, isModalVisible: false, modalJSX: null, payload: null});
  };
  ...
  const submitAddress = async () => {
    ...
    try {
      await axios.post('/api/profile', {currentAddress: fieldCurrAddress}, config);
      ...
      modalJSX: payload && (
        <Fragment>
          ...
          {payload.mobileNumber && (
            <p>
              <strong>Mobile Number:</strong> {payload.mobileNumber}
            </p>
          )}
          {payload.permanentAddress && (
            <p>
              <strong>Permanent address:</strong> {payload.permanentAddress}
            </p>
          )}
          {payload.spouseName && (
            <p>
              <strong>Spouse:</strong> {payload.spouseName}
            </p>
          )}
          {payload.alternateContactNumber && (
            <p>
              <strong>Alt-Contact Number:</strong> {payload.alternateContactNumber}
            </p>
          )}
          {payload.occupation && (
            <p>
              <strong>Occupation:</strong> {payload.occupation}
            </p>
          )}
          {payload.sourceOfIncome && (
            <p>
              <strong>Source of income:</strong> {payload.sourceOfIncome}
            </p>
          )}
          {payload.company && (
            <p>
    ...
    <Fragment>
      ...
      {isEditEnabled && (
        <div className="profile-submit">
          <button className="btn btn-primary" onClick={() => submitInfo(_id)} disabled={dateDiff || !isValidMobNumb || !isValidAltCont}>
            Request for update
          </button>
        </div>
      )}
...
export default connect(null, {setAlert, getCurrentProfile})(ShowProfile);
